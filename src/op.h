...
typedef enum { CJ_REGISTER, CJ_CONSTANT, CJ_MEMORY, CJ_REGISTER_LIST } cj_operand_type;
...
typedef enum { CJ_MEM_MODE_OFFSET = 0, CJ_MEM_MODE_PRE, CJ_MEM_MODE_POST } cj_mem_mode;
...
static inline cj_operand cj_make_memory(const char *base, const char *index, uint8_t scale,
                                        int32_t disp) {
  cj_operand op = {
      .type = CJ_MEMORY,
      .mask = NULL,
      ...
      .rounding = CJ_ROUND_DEFAULT,
      .mem =
          {.base = base, .index = index, .scale = scale, .disp = disp, .mode = CJ_MEM_MODE_OFFSET},
      .shift = {.kind = CJ_SHIFT_KIND_NONE, .amount = 0, .has_amount = 0},
      .extend = {.kind = CJ_EXTEND_KIND_NONE, .amount = 0, .has_amount = 0},
  };
  return op;
